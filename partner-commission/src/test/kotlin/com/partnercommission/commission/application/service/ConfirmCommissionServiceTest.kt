...
class ConfirmCommissionServiceTest {

    private val commissionRepository: CommissionRepository = mockk()
    private val service = ConfirmCommissionService(commissionRepository)
    ...
    fun `정상 확정`() {
        ...
        val result = service.execute(ConfirmCommissionCommand(tenantId, attrId))

        assertThat(result.currentStatus()).isEqualTo(CommissionStatus.CONFIRMED)
    }
    ...
    fun `커미션 없으면 예외`() {
        ...
        assertThatThrownBy { service.execute(ConfirmCommissionCommand(tenantId, attrId)) }
